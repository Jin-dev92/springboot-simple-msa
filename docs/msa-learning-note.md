...
이 절의 대가 역시 18절의 문장으로 모입니다. 조정자는 **나누었기 때문에 흩어진 흐름**을 다시 한 곳으로 모으려고 존재합니다.
...
복제로 풀리지 않는 조회가 있습니다. 조건은 하나입니다 — **상대 서비스의 "지금 상태"가 조회 조건이나 정렬 기준에 들어갈 때**입니다.

| 질의 | 복제로 되나 | 왜 |
|---|---|---|
| 내 주문 목록을 금액 큰 순으로 정렬해 페이징 | **된다** | `totalPrice`·`productName` 모두 주문 시점 값이라 `orders` 안에 있다 |
| 내가 주문한 상품 중 **지금 품절인 것** | 안 된다 | `stock`은 현재 상태다. 박제하면 낡는다 |
| **카테고리별** 매출 집계 | 안 된다 | `category`는 product의 현재 속성이다 |
| 재고 10개 미만 상품의 미처리 주문 | 안 된다 | 두 서비스의 현재 상태를 함께 건다 |

첫 줄이 중요합니다. 정렬·페이징이라고 해서 자동으로 읽기 모델이 필요한 것은 아닙니다. **기준이 되는 값이 이미 내 DB에 있으면 평범한 SQL로 끝납니다.** 복제가 조회 문제의 상당 부분을 먼저 걷어낸다는 뜻입니다.

아래 다섯 가지는 그러고도 남는 경우의 선택지입니다. **이 저장소가 실제로 구현한 것은 복제(위)까지이고, 아래는 개념 정리입니다.**

#### 방법 1. API Composition — 호출자가 합친다

여러 서비스를 부르고 메모리에서 붙입니다. 게이트웨이나 BFF에 두면 클라이언트는 몰라도 됩니다.

N+1은 벌크 엔드포인트로 완화합니다. 주문 20건에 상품 조회 20번이 아니라 id를 모아 한 번에 묻습니다.

```
GET /products?ids=1,3,7,12      → 호출 1회
```

추가 저장소도, 일관성 문제도 없고 항상 최신입니다. **한계는 정렬·필터·페이징입니다.** 정렬 기준이 상대 서비스에 있으면 전부 가져와 메모리에서 정렬해야 하므로, 데이터가 늘면 무너집니다.

#### 방법 2. 참조 데이터 복제 — 로컬 사본을 계속 동기화

스냅샷과 헷갈리기 쉬운데 **정반대**입니다. 시점을 고정하는 것이 아니라 **계속 갱신**합니다.

order-service가 `product_replica` 테이블을 두고 product-service의 변경 이벤트를 구독해 갱신하면, 자기 DB 안에서 **진짜 SQL 조인**이 가능해집니다.

```sql
select o.*, p.category
from orders o join product_replica p on o.product_id = p.id
where p.category = '주변기기' order by o.total_price desc limit 20;
```

상대 데이터가 **작고 잘 바뀌지 않을 때** 적합합니다 — 카테고리, 코드값, 국가 목록 같은 것들입니다. 대가는 결과적 일관성이고, 이벤트를 놓치면 조용히 낡으므로 재동기화 경로가 필요합니다.

#### 방법 3. CQRS 읽기 모델 — 이벤트 기반 구체화 뷰

**구체화 뷰(materialized view)** 는 질의 결과를 물리적으로 저장해 둔 것입니다. 일반 뷰가 조회할 때마다 계산하는 것과 달리, 읽기 속도를 얻는 대신 저장 공간·갱신 비용·신선도를 지불합니다.

단일 DB라면 원본 테이블이 바뀔 때 DB가 알아서 갱신합니다. **그런데 서비스를 나누면 원본 테이블에 SQL이 닿지 않습니다.** 그래서 갱신 트리거를 SQL이 아니라 **이벤트**로 바꿉니다. 이것이 이벤트 기반 구체화 뷰입니다.

```mermaid
flowchart LR
    subgraph w ["쓰기 쪽 (각자의 DB)"]
        o[("order-service<br/>orders")]
        p[("product-service<br/>products")]
    end

    k[["Kafka"]]

    subgraph r ["읽기 쪽"]
        v[("order_summary<br/>비정규화 · 조인 완료")]
    end

    o ==>|"OrderConfirmed"| k
    p ==>|"ProductRenamed<br/>StockChanged"| k
    k ==>|"구독 → UPSERT"| v
    v -->|"조회 전용"| c(["client"])
```

읽기 쪽 테이블은 조인이 미리 끝난 모양이 됩니다.

```
order_summary { orderId, userId, productId, productName, category,
                quantity, totalPrice, status, currentStock, orderedAt }
```

**이름 셋을 구분해 두면 좋습니다.** 한 덩어리로 불리지만 각각 다른 것입니다.

| 용어 | 무엇인가 | 없어도 되나 |
|---|---|---|
| CQRS | 쓰기 모델과 읽기 모델의 분리 | 같은 DB 안에서도 성립. 별도 저장소가 조건이 아니다 |
| 구체화 뷰 | 조회 결과를 물리적으로 저장 | 이벤트 없이 배치로 갱신해도 된다 |
| 이벤트 소싱 | 상태 자체를 이벤트 로그로 저장 | 구체화 뷰와 별개. 다만 있으면 재구축이 쉽다 |

**어려운 부분은 만드는 게 아니라 유지하는 것입니다.** 만드는 것은 리스너 하나입니다. 진짜 비용은 셋입니다.

**첫째, 누락이 영구적입니다.** 10~11절에서 본 것과 결정적으로 다릅니다.

| | 재고 (Saga) | 구체화 뷰 |
|---|---|---|
| 이벤트를 놓치면 | 다음 명령이 다시 오고, 타임아웃 스위퍼가 걷어낸다 | **아무도 모른다** |
| 자기 교정 | 있다 | **없다** |
| 증상 | 실패가 드러난다 | 조용히 틀린 값을 계속 내놓는다 |

그래서 **Transactional Outbox가 사실상 선행조건**입니다. 로드맵에서 Outbox를 CQRS보다 앞에 둔 이유가 이것입니다.

**둘째, 재구축 경로가 필요합니다.** 뷰가 어긋났거나 스키마를 바꾸면 처음부터 다시 만들어야 합니다.

```
새 컨슈머 그룹 → 오프셋 0부터 재소비 → 새 테이블에 적재
→ 따라잡으면 조회 대상을 교체 (blue-green)
```

전제가 하나 붙습니다. **Kafka에 과거 이벤트가 남아 있어야 합니다.** 기본 보존 기간은 7일이므로, 그보다 오래된 것을 복원하려면 보존을 늘리거나(compacted topic) 원본 서비스에서 다시 긁어오는 경로를 따로 두어야 합니다. 이벤트 소싱이 여기서 편해집니다 — 로그 자체가 진실의 원천이라 재구축이 자연스럽습니다.

**셋째, 자기가 쓴 것을 자기가 못 읽습니다.** 주문하고 목록으로 돌아왔는데 행이 아예 없습니다. Saga의 `PENDING`은 "상태가 아직 확정되지 않음"이었지만, 여기서는 **존재 자체가 늦습니다.** 쓰기 응답으로 화면을 낙관적으로 그리거나, 방금 쓴 사용자만 잠시 쓰기 쪽에서 읽는 식으로 다룹니다.

#### 방법 4. CDC + 분석 저장소

Debezium 같은 도구로 각 서비스 DB의 변경 로그를 빨아 웨어하우스에 모읍니다. **리포팅·BI·집계는 이쪽이 정석이며**, 운영 API에 CQRS를 씌우는 것과 목적이 다릅니다.
...
"매출 집계 화면 때문에 CQRS가 필요하다"는 대개 이쪽으로 풀립니다.

#### 방법 5. 서비스 경계 재검토

**조인이 자주 필요하다는 것은 두 데이터의 응집도가 높다는 신호입니다.** 경계가 잘못 그어졌을 수 있습니다.

- 이 둘은 **항상 함께 조회**되는가
- **함께 변경**되는가
- 따로 배포할 실익이 정말 있는가

셋 다 예라면 기술로 조인을 흉내 내기보다 **두 서비스를 합치는 편이 답인 경우가 많습니다.** 조인 문제를 푸는 가장 저렴한 방법은 조인이 필요 없게 경계를 다시 긋는 것입니다.

### 고르는 순서

```mermaid
flowchart TB
    A["붙이려는 값이<br/>과거의 사실인가?"] -->|예| B["복제 · 스냅샷"]
    A -->|"아니오 (현재 상태)"| C["정렬·필터·페이징이<br/>필요한가?"]
    C -->|아니오| D["API Composition<br/>+ 벌크 엔드포인트"]
    C -->|예| E["상대 데이터가<br/>작고 잘 안 바뀌는가?"]
    E -->|예| F["참조 데이터 복제<br/>→ 로컬 SQL 조인"]
    E -->|아니오| G["운영 조회인가<br/>분석 조회인가?"]
    G -->|운영| H["CQRS 읽기 모델<br/>(Outbox 선행)"]
    G -->|분석| I["CDC + 웨어하우스"]
```

어느 가지에 서든, **자주 걸린다면 방법 5를 한 번 거쳐야** 합니다.

### 하면 안 되는 것 — 공유 데이터베이스

여러 서비스가 같은 DB를 보고 스키마를 가로질러 조인하는 것입니다. SQL 한 줄로 끝나므로 유혹이 큽니다.

잃는 것은 이렇습니다. 테이블 구조를 바꾸려면 **누가 내 테이블을 조인하고 있는지 전부 찾아야** 하고, 결국 함께 배포해야 하며, 그러면 서비스를 나눈 이유가 사라집니다.

> 나눠 놓고 조인으로 다시 묶으면 **분산 모놀리식**이 됩니다. 모놀리식의 결합도에 분산의 운영 비용을 더한 조합입니다.

분석용으로 읽기 전용 복제본을 여는 타협은 현실에서 봅니다. 다만 스키마 결합은 그대로 남으므로 방법 4가 낫습니다.

### 이 저장소의 현재 위치

복제(스냅샷)까지 왔습니다. 그리고 **그 결과 방법 1~4를 정당화하는 질의가 아직 없습니다.** 위 표의 첫 줄처럼, 필요해 보이던 조회가 복제 후에는 로컬 SQL로 끝나기 때문입니다.

읽기 모델을 얹으려면 표의 아래 세 줄 같은 화면을 **먼저 만들어서** API Composition으로 시도하고, 정렬·페이징이 막히는 것을 손으로 확인하는 순서가 좋습니다. 그래야 "조인이 안 돼서 도입했다"가 아니라 정확한 이유가 남습니다.

---

## 13. Transactional Outbox — 커밋과 발행을 하나로 묶기

### 문제

11절에서 발행 시점을 커밋 뒤로 옮겼습니다. 트랜잭션 안에서 보내면 롤백돼도 명령이 이미 나가 있기 때문이었습니다. 그런데 순서를 바꿔도 창은 남습니다.
...
**상태는 바뀌었는데 명령이 나가지 않습니다.** 사가는 대기 단계에 멈춰 있고, 참여자는 아무것도 받지 못합니다.

이 창을 없앨 수 없는 이유는 근본적입니다. **DB와 Kafka는 다른 시스템이고, 두 시스템에 걸친 트랜잭션이 없습니다.** 어느 쪽을 먼저 하든 사이에 죽으면 어긋납니다. 11절의 선택은 **덜 나쁜 쪽**이었을 뿐입니다.

| 순서 | 사이에 죽으면 | 되돌릴 수 있나 |
|---|---|---|
| 발행 → 커밋 | 명령은 나갔는데 상태가 없다 | **없다.** 참여자가 이미 실행했다 |
| 커밋 → 발행 | 상태는 있는데 명령이 없다 | 있다. 스위퍼가 걷어낸다 |

### 해법: 메시지도 DB에 쓴다

브로커에 보내는 대신 **같은 DB의 테이블에** 한 줄 넣습니다. 그러면 상태 변경과 메시지가 하나의 트랜잭션에 들어갑니다. **둘 다 커밋되거나 둘 다 롤백됩니다.** 창이 사라집니다.
...
flowchart LR
    ...
    tx --> db[("order DB")]
    db -.->|"주기적으로 조회"| r["OutboxRelay"]
    r ==>|"발행 후 published 표시"| k[["Kafka"]]
```

**쓰는 쪽은 브로커의 사정을 알 필요가 없습니다.** Kafka가 죽어 있어도 주문은 정상적으로 커밋됩니다.

### 무엇을 무엇과 바꾸었는가

공짜가 아닙니다. 이 패턴의 요지는 여기 있습니다.

> **유실 가능성을 지연과 중복 가능성으로 바꿉니다.**

- **지연** — 릴레이 주기(여기서는 1초)만큼 발행이 늦어집니다.
- **중복** — 발행에 성공하고 `published` 표시 직전에 죽으면 다음 주기가 다시 보냅니다.

중복이 받아들일 만한 이유는 **참여자가 이미 멱등**하기 때문입니다(10절). `(orderId, action)` 기록이 두 번째 명령을 흡수합니다. **먼저 멱등을 만들어 두었기 때문에 이 교환이 성립합니다.**
...
**첫째, 전송이 끝날 때까지 기다린 뒤에 표시합니다.** 비동기로 던져 놓고 바로 표시하면, 전송이 나중에 실패해도 "보냈다"고 기록되어 메시지가 사라집니다. 유실을 막으려고 만든 구조가 유실을 만듭니다.

**둘째, 실패하면 거기서 멈춥니다.** 다음 것으로 건너뛰면 순서가 깨집니다. 같은 주문의 `RELEASE`가 `RESERVE`를 추월하면 재고가 어긋납니다. 남은 것은 다음 주기가 그 자리부터 이어 갑니다.

### 코드

| 파일 | 역할 |
|---|---|
| `order-service/.../OutboxMessage.java` | 발행 대기 메시지. `publishedAt` 이 `null` 이면 미발행 |
| `order-service/.../OutboxRepository.java` | 미발행 메시지를 **id 오름차순**으로 조회 |
| `order-service/.../OutboxRelay.java` | `@Scheduled` 로 꺼내 발행하고 표시 |
| `OrderSagaOrchestrator.toOutbox()` | 발행 대신 테이블에 쓴다. `sendAfterCommit` 을 대체 |

릴레이는 폴링입니다. 실무의 정석은 CDC(Debezium 등)로 DB 변경 로그를 직접 빨아 발행하는 것이지만, 여기서는 스위퍼와 같은 모양이라 새로 배울 인프라가 없고 **"상태와 메시지를 한 트랜잭션에 쓴다"** 는 핵심에 집중할 수 있어 폴링을 택했습니다.

### 직접 확인

Kafka를 멈춘 채 주문합니다. **이전 구조라면 명령이 유실되어 30초 뒤 취소됐을 상황입니다.**
...
실측 결과입니다. 주문이 **정상 접수**됩니다.

```
{"id":2,"productName":"키보드","totalPrice":178000.00,"status":"PENDING"}
```

릴레이는 발행에 실패하며 재시도합니다.

```
order | outbox 발행 실패. 순서를 지키기 위해 이번 주기를 여기서 멈춘다 (id=3 ...)
```

Kafka를 살리면 밀린 명령이 나가고 사가가 끝까지 진행됩니다.

```
docker compose start kafka

{"id":1,"status":"CONFIRMED","cancelReason":null}
{"id":2,"status":"CONFIRMED","cancelReason":null}

payment | 결제 완료: userId=1, 청구액=178000.00, 남은잔액=733000.00 (orderId=2)
```

재고 27, 잔액 733,000원 모두 맞습니다. **스위퍼는 한 번도 개입하지 않았습니다.**

### 스위퍼와의 관계 — 중단이 길면 어떻게 되는가

위 실측에서 Kafka 중단은 약 12초였습니다. Saga 타임아웃 임계인 30초보다 짧아 스위퍼가 뜨기 전에 복구됐습니다.

**중단이 30초를 넘으면 스위퍼가 먼저 주문을 취소합니다.** Outbox가 보장하는 것은 **메시지가 언젠가 반드시 나간다**는 것이지, 사가가 무한히 기다린다는 것이 아닙니다. 둘은 다른 장치이며 목적도 다릅니다.

| | Outbox | 타임아웃 스위퍼 |
|---|---|---|
| 막는 것 | 메시지 유실 | 무한 대기 |
| 판단 | 아직 안 보냈다 → 보낸다 | 너무 오래 걸렸다 → 되돌린다 |

임계를 늘리면 더 긴 중단을 견디지만, 그만큼 진짜로 멈춘 주문도 오래 방치됩니다. **어느 쪽 실패를 더 오래 참을 것인가의 선택**입니다.

### 남은 것

**참여자(product·payment)는 아직 리스너에서 직접 발행합니다.** 같은 패턴을 적용한 구간과 그렇지 않은 구간을 나란히 두어 비교할 수 있게 남겼습니다. 참여자 쪽 유실은 멱등 기록 덕에 재전송으로 복구되지만, 재전송을 촉발할 것이 없으면 응답이 그대로 사라집니다 — 그때는 스위퍼가 걷어냅니다.

**발행이 끝난 행은 계속 쌓입니다.** 인메모리 H2라 재시작하면 사라지므로 이 저장소에서는 문제가 되지 않지만, 영속 DB라면 오래된 발행분을 주기적으로 지워야 합니다.

---

## 14. 참조 데이터 복제 — 남의 현재 상태로 정렬하기

### 문제

12절의 스냅샷으로 조인 통증의 상당 부분이 사라졌습니다. 그런데 이런 화면은 여전히 안 됩니다.

> **내 주문 목록을 상품의 현재 재고가 적은 순으로 정렬해서 20개씩 보여 주세요.**

`productName`은 박제해 두었지만 `stock`은 다릅니다. **"지금 품절인가"는 주문 시점 재고로 답할 수 없습니다.**

API Composition으로 해 보면 벽이 어디인지 분명해집니다.

```
1. 내 주문을 전부 가져온다          ← 페이징을 못 한다. 정렬 기준이 아직 없다
2. 상품을 전부 가져온다             ← 벌크 조회로 N+1은 피할 수 있다
3. 메모리에서 붙인다
4. 메모리에서 정렬한다
5. 메모리에서 20개를 잘라낸다
```

N+1은 벌크 엔드포인트로 풀리지만 **1번은 풀리지 않습니다.** 정렬 기준이 상대 서비스에 있으면 **페이징을 DB로 내려보낼 수 없습니다.** 20개를 보여 주려고 전부를 메모리에 올려야 하고, 주문이 늘수록 그대로 무너집니다.

### 해법: 상대 데이터를 내 DB에 복제한다

`order-service`가 `product_replica` 테이블을 두고, product-service의 변경 이벤트를 구독해 갱신합니다. 그러면 자기 DB 안에서 **평범한 SQL 조인**이 됩니다.

```sql
select o.*, r.stock
from orders o left join product_replica r on r.product_id = o.product_id
where o.user_id = ?
order by r.stock asc nulls last
limit 20 offset 0
```

**정렬도 페이징도 DB가 합니다.** 12절 결정 트리의 "상대 데이터가 작고 잘 안 바뀌는가 → 참조 데이터 복제" 가지입니다.

### 스냅샷과 정반대입니다

12절과 이 절은 둘 다 "남의 데이터를 내 DB에 둔다"지만 **성격이 정반대**입니다. 헷갈리면 둘 다 틀립니다.

| | 스냅샷 (12절) | 복제본 (14절) |
|---|---|---|
| 예 | `Order.productName` | `ProductReplica.stock` |
| 원본이 바뀌면 | **따라가지 않는다** | **따라간다** |
| 답하는 질문 | 그때 무엇이었나 | 지금 무엇인가 |
| 틀리는 경우 | 현재 상태를 박제했을 때 | 과거 사실을 계속 덮었을 때 |

`OrderSummary` 한 줄 안에 둘이 함께 있습니다. `productName`은 주문 시점 값이고 `currentStock`은 지금 값입니다.

### 발행 쪽 — 여기서 Outbox가 전제가 됩니다

product-service가 `ProductChangedEvent`를 새로 발행해야 합니다. 그런데 **리스너에서 직접 발행하면 안 됩니다.**

13절에서 order-service에 Outbox를 넣은 이유와 여기 이유가 다릅니다.

| | order-service (13절) | product-service (14절) |
|---|---|---|
| 메시지를 놓치면 | 사가가 멈춘다 | 복제본이 낡은 값을 든다 |
| 누가 알아채나 | **타임아웃 스위퍼** | **아무도** |
| 자기 교정 | 있다 | **없다** |

**복제본을 두는 순간 발행 신뢰성이 선택이 아니라 전제가 됩니다.** 그래서 product-service에도 Outbox를 먼저 넣었습니다. 코드는 order-service 것과 같은 모양이며, 공통 모듈을 두지 않는 방침에 따른 의도된 중복입니다.

### 이벤트에 상품 전체를 싣는 이유

바뀐 필드만 보내지 않고 상품의 현재 모습 전체를 보냅니다.

```java
public record ProductChangedEvent(Long productId, String name, BigDecimal price, int stock)
```

받는 쪽은 이전 상태를 몰라도 **통째로 덮어쓰면**(upsert) 됩니다. 그 덕에 **이벤트를 몇 개 놓쳤다가 최신 것 하나만 받아도 결국 맞는 값으로 수렴**합니다. 증분(`stock -= 3`)으로 보냈다면 하나만 놓쳐도 영영 어긋납니다.

### 구독 쪽에서 부딪힌 것 둘

**1) 한 서비스가 두 종류의 메시지를 받으면 기본 타입 하나로는 안 됩니다.**

`order-service`는 이제 `saga-reply`와 `product-changed`를 함께 구독합니다. 발신측이 타입 헤더를 붙이지 않는 방침이라 수신측이 타입을 정해야 하는데, `spring.json.value.default.type`은 값이 하나뿐입니다. 그래서 새 리스너만 다른 역직렬화기를 쓰는 컨테이너 팩토리를 따로 두었습니다(`KafkaConsumerConfig`).

역직렬화기를 코드로 만들면서 `spring.json.*` 설정을 함께 넘기면 기동 시점에 거부당합니다.
...
**공통 모듈을 두지 않는 대가를 여기서 한 번 더 지불하는 셈**입니다. 타입 헤더를 켜면 이 설정이 사라지지만, 그러면 수신측이 발신측과 같은 패키지·클래스 이름을 갖도록 강요받습니다.

**2) `KafkaTemplate` 빈을 하나 선언하면 자동 설정이 물러납니다.**

릴레이용 템플릿을 추가하는 순간 스프링 부트의 기본 템플릿이 사라져(`@ConditionalOnMissingBean`) 기존 리스너가 주입 대상을 잃습니다. **하나를 직접 만들면 나머지도 직접 만들어야 합니다.** `spring.kafka.template.observation-enabled` 도 자동 설정용이라 직접 켜야 하고, 빠뜨리면 Zipkin 추적이 조용히 끊깁니다.

### 코드

| 파일 | 역할 |
|---|---|
| `product-service/.../ProductChangedEvent.java` | 상품의 현재 모습 전체 |
| `product-service/.../ProductEventPublisher.java` | 재고 변경과 **한 트랜잭션**으로 outbox 기록 |
| `product-service/.../Outbox*.java` | 13절과 같은 구조 (의도된 중복) |
| `order-service/.../ProductReplica.java` | 로컬 사본. `apply()` 로 덮어쓴다 |
| `order-service/.../ProductChangedListener.java` | 이벤트 수신 → upsert |
| `order-service/.../KafkaConsumerConfig.java` | 두 번째 타입을 위한 컨테이너 팩토리 |
| `OrderRepository.findSummariesByUserId` | 조인 + 재고 정렬 + 페이징 |

### 직접 확인

세 상품을 주문한 뒤 요약을 조회합니다.

```bash
curl -s "http://localhost:8080/api/orders/summary" -H "Authorization: Bearer $TOKEN"
```

**주문한 순서가 아니라 현재 재고 순으로 나옵니다.** 원본과 값도 일치합니다.

```
원본:   키보드 29,  모니터 11,  마우스 49
요약:   {"productName":"모니터","currentStock":11}
        {"productName":"키보드","currentStock":29}
        {"productName":"마우스","currentStock":49}
```

페이징이 DB에서 처리되므로 페이지를 가로질러도 정렬이 이어집니다.

```
page 0 (size=2):  {"total":3, "pages":2, "items":[11,29]}
page 1 (size=2):  {"items":[49]}
```

**product-service를 정지시켜도 이 화면은 뜹니다.**

```
GET /api/products/1        → 500
GET /api/orders/summary    → 모니터 11 / 키보드 29 / 마우스 49  (stockUpdatedAt 포함)
```

### 대가 — 복제본은 낡습니다

마우스 5개를 주문하고 바로 조회하면 **아직 옛 값**입니다.

```
주문 직전:  49
주문 직후:  49      ← 아직 이벤트가 도착하지 않았다
8초 뒤:     45
원본:       45
```

`stockUpdatedAt`을 응답에 함께 실어 **얼마나 낡았는지를 숨기지 않습니다.** 결과적 일관성을 쓰는 화면은 그 사실을 드러내는 편이 낫습니다.

추적도 한 번 끊깁니다. outbox를 거치는 순간 발행이 원래 요청과 다른 스레드·다른 시점으로 떨어지므로, Zipkin에서 그 span은 릴레이에 붙습니다.

### 그래서 이것은 CQRS인가

**기제는 CQRS 읽기 모델과 같고, 다른 것은 저장 위치뿐입니다.**

- 쓰기 모델(product-service의 `products`)과 읽기 모델(`product_replica`)의 분리
- 이벤트로 동기화
- 결과적 일관성
- 누락이 영구 오류가 되는 성질과, 그래서 필요한 발행 신뢰성

별도 저장소·서비스로 떼어내면 **운영해야 할 인프라가 늘 뿐 개념은 여기서 다 나옵니다.** 상품이 3행이고 서비스를 가로지르는 화면이 하나인 지금 규모에서는 이쪽이 맞습니다. 화면이 여러 개로 늘고 조회 부하를 따로 감당해야 할 때가 옮길 시점입니다.

---

## 15. 복제본 재구축 — 자기 교정이 없는 것을 되돌리기

### 문제: 추측이 아니라 재현된 결함이었습니다

14절 끝에 "복제본 재구축"을 나중 과제로 미뤄 두고, 16절에는 이렇게 적었습니다.

> 인메모리 H2 라 재시작하면 어차피 처음부터 다시 쌓인다

**틀렸습니다.** 확인해 보니 명령 하나로 재현됩니다.
...
`null`이 "아직 못 받았다"가 아니라는 것은 **재고 부족 주문**으로 분리해 확인했습니다. 재고가 바뀌지 않으니 새 이벤트가 발생하지 않고, 그래도 값이 없다면 복제본에 그 상품이 **아예 없다**는 뜻입니다.

`docker compose down && up`은 Kafka까지 새로 뜨면서 오프셋이 사라지므로 괜찮습니다. **한 서비스만 재시작할 때 드러납니다.**

### 원인: 수명 불일치

```
복제본      →  서비스와 함께 죽는다      (인메모리 H2, create-drop)
컨슈머 오프셋 →  브로커에 남는다          (order-service 밖)

빈 테이블  +  "이미 다 읽었다"  =  그 상태로 굳는다
```

인메모리 H2가 이를 과장해 보이게 할 뿐, **영속 DB에서도 형태는 같습니다.** 복제본만 잘못 지워지거나 마이그레이션에서 날아간 경우가 그렇습니다. 고칠 방법은 둘입니다.

| | 방법 | 이 프로젝트 |
|---|---|---|
| 수명을 맞춘다 | 복제본을 오프셋만큼 오래 살게 (영속 DB) | 학습 범위 밖 |
| 되돌릴 수단을 둔다 | **재구축** | 이쪽 |

### 재구축이 가능하려면 — 압축 토픽

토픽을 처음부터 다시 읽으려면 **과거 이벤트가 남아 있어야** 합니다. 기본 보존은 7일이라 그 이전 것은 없습니다.

`product-changed`를 **압축(compact)** 으로 바꿉니다.

```java
TopicBuilder.name(ProductChangedEvent.TOPIC).partitions(1).replicas(1).compact().build()
```

Kafka가 **키별 최신 메시지만** 남깁니다. 키가 `productId`이므로 상품이 3개면 메시지 3개가 영원히 남습니다. 보존 기간을 무한으로 늘리는 것과 다른 점은 **저장량이 변경 횟수가 아니라 상품 수에 비례**한다는 것입니다.

이 토픽이 "사실의 누적"이 아니라 **"현재 상태의 스냅샷"** 이라 압축과 성격이 맞습니다. 14절에서 이벤트에 상품의 현재 모습 전체를 실은 결정이 여기서 값을 합니다 — 중간 것을 버리고 최신 하나만 남겨도 복제본은 맞는 값으로 수렴합니다.

> 명령·응답 토픽은 압축하면 안 됩니다. 같은 주문의 RESERVE와 RELEASE가 같은 키를 쓰므로 하나가 사라집니다. **압축은 "키별 최신 상태"에만 맞습니다.**

### 리스너의 컨슈머 그룹을 건드리지 않습니다

재구축이 평상시 소비와 얽히면 안 됩니다. 그룹 오프셋을 되감으면 리스너가 과거 이벤트를 다시 처리하고, 재구축을 여러 번 돌릴수록 부작용이 쌓입니다.

그래서 `subscribe`가 아니라 **`assign`** 으로 파티션을 직접 잡습니다.
...
`assign`은 그룹 관리에 참여하지 않고 오프셋도 커밋하지 않습니다. **몇 번을 돌려도 평상시 흐름에 흔적이 남지 않습니다.** 실측에서도 리스너 그룹은 `CURRENT 2 / END 2, LAG 0`을 그대로 유지했습니다.

### 두 개의 트리거

| 트리거 | 언제 | 다루는 상황 |
|---|---|---|
| 기동 시 자동 | 복제본이 **비어 있을 때만** | 위에서 재현한 결함 |
| 관리 엔드포인트 | 사람이 부를 때 | **비어 있지 않지만 틀린** 경우 |

두 번째가 따로 필요한 이유가 있습니다. 이벤트를 놓쳤거나, 리스너 버그로 잘못 반영했거나, 복제본 스키마를 바꿨을 때는 테이블이 비어 있지 않습니다. 자동 트리거만으로는 손댈 수 없습니다.

기동 시 재구축은 **실패해도 기동을 막지 않습니다.** 브로커가 아직 안 떴을 뿐일 수 있고, 그때 서비스를 통째로 못 뜨게 하는 것은 과합니다. 대신 경고를 남깁니다 — 복제본이 빈 채로 도는 것은 조용히 넘어가면 안 되는 상태입니다.

### 코드

| 파일 | 역할 |
|---|---|
| `order-service/.../ProductReplicaRebuilder.java` | 토픽을 처음부터 읽어 upsert. `assign` 으로 그룹과 분리 |
| `order-service/.../ProductReplicaStartupRebuild.java` | 기동 시 비어 있으면 재구축 |
| `order-service/.../ProductReplicaAdminController.java` | 수동 트리거 (ADMIN) |
| `ProductServiceApplication.productChangedTopic` | 압축 토픽 선언 |
...
```
c.e.msa.order.ProductReplicaRebuilder     : 상품 복제본 재구축 완료: 2건
c.e.m.o.ProductReplicaStartupRebuild      : 복제본이 비어 있어 재구축했다: 2건
```

같은 결정적 테스트(재고 부족 주문)로 확인하면 값이 살아 있습니다.

```
{"productName":"모니터","currentStock":11,"stockUpdatedAt":"..."}
원본: {"name":"모니터","stock":11}
```
...
리스너 그룹: CURRENT 2 / END 2, LAG 0    ← 오프셋이 되감기지 않았다
```

### 대가

재구축은 **토픽 전체를 다시 읽습니다.** 압축 덕에 상품 수만큼이지만, 데이터가 큰 시스템에서는 그렇지 않습니다. 그때 필요한 것이 blue-green입니다 — 새 테이블에 쌓아 따라잡은 뒤 조회 대상을 바꾸면 재구축 중에도 화면이 옛 복제본으로 계속 뜹니다. 여기서는 복제본이 3행이라 넣지 않았습니다.

그리고 재구축은 **복제본이 틀렸다는 것을 누군가 알아챘을 때** 쓸 수 있습니다. 알아채는 것 자체는 별개 문제이며, 원본과 복제본을 주기적으로 대조하는 장치는 이 프로젝트에 없습니다.

---

## 16. 복제본 검증 — 틀렸다는 것을 알아채기

### 문제

15절에서 재구축 수단을 만들었지만 빠진 것이 있었습니다. **틀렸다는 것을 누가 알아채는가.** 사람이 눈치채야 재구축을 부를 수 있었습니다.

복제본은 세 가지 이유로 틀어지는데, **하나의 감시로는 다 잡히지 않습니다.**

| 원인 | 갭 감지로 | 값 대조로 |
|---|---|---|
| 이벤트 유실 | **잡힌다** | 잡힌다 |
| 잘못 반영 (리스너 버그, 매핑 실수) | 안 잡힌다 | **잡힌다** |
| 마지막 이벤트 유실 | **안 잡힌다** | **잡힌다** |

세 번째가 특히 중요합니다. 이벤트가 전체 상태를 싣기 때문에(14절) 중간을 놓쳐도 다음 것 하나로 복구되지만, **마지막 것을 놓치면 다음이 오지 않아 영원히 낡습니다.** 갭은 "다음 이벤트"가 와야 드러나므로 이 경우를 볼 수 없습니다.

그래서 둘을 함께 둡니다.

### A. 갭 감지 — 놓쳤는가

상품에 **변경 순번**을 붙여 이벤트에 싣고, 복제본이 마지막으로 반영한 순번을 기록합니다.

```java
if (replica.alreadyApplied(event.version())) return;      // 중복·순서 역전
if (replica.hasGapBefore(event.version())) log.warn(...); // 놓친 것이 있다
replica.apply(..., event.version());
```
...
| 판정 | 조건 | 하는 일 |
|---|---|---|
| 이미 봄 | `받은 ≤ 보유` | 무시. **중복 전달과 순서 역전이 함께 걸린다** |
| 갭 | `받은 > 보유 + 1` | 경고를 남기고 값은 반영 |
| 정상 | `받은 = 보유 + 1` | 반영 |

값 자체는 갭이 있어도 이 이벤트로 맞춰집니다. **그래도 경고를 남기는 이유는 유실이 일어나고 있다는 신호이기 때문입니다.** 마지막 이벤트를 놓치는 상황의 전조입니다.

> JPA의 `@Version`(낙관적 락)을 쓰지 않고 직접 둡니다. 낙관적 락은 동시 수정을 막으려는 것이고 이 값은 **구독자에게 순서를 알려 주려는** 것입니다. 하나를 두 목적에 겹쳐 쓰면 나중에 락 전략을 바꿀 때 구독자 쪽 의미까지 흔들립니다.

### B. 값 대조 — 같은가

원본과 복제본의 내용을 비교합니다. 전부 내려받아 필드별로 보면 상품 수만큼 데이터가 오가므로, **체크섬**으로 요약합니다.

```
GET /products/checksum → {"count":3, "checksum":"d83687fc..."}
```

왕복 한 번으로 전체가 같은지 알 수 있습니다. 대신 **어디가** 다른지는 모르므로, 다르면 전체 재구축이 답이 됩니다.

### 진짜 어려운 부분은 비교가 아니라 오탐입니다

**결과적 일관성 시스템에서는 정상 상태에서도 두 값이 다릅니다.**
...
이걸 그대로 경보로 올리면 늘 울려서 아무도 보지 않게 됩니다. 판정 기준을 이렇게 둡니다.

> **단발 불일치는 정상입니다. 지속되는 불일치만 결함입니다.**

연속 3회 다를 때만 경고합니다. 일시적 지연이면 다음 회차에 사라지고, 진짜로 어긋났으면 남습니다. **검사 주기를 복제 지연보다 넉넉히 크게 잡는 것이 전제**입니다.

한 가지 더 구분합니다. 원본을 **조회하지 못한 것**은 불일치가 아닙니다.

```java
if (origin == null) return;   // "틀렸다"가 아니라 "확인할 수 없다"
```

서킷 브레이커 폴백이 여기서 `findById`와 정반대로 동작합니다. 가격은 지어낼 수 없으니 주문을 거절해야 하지만, 체크섬 조회 실패는 검증을 건너뛰면 됩니다. **확인 못 한 것을 결함으로 세면 상대의 장애가 곧바로 거짓 경보가 됩니다.**

### 자동 재구축은 하지 않습니다

수단은 15절에서 이미 만들었고 연결도 쉽습니다. 그런데도 하지 않습니다. **오탐에 반응해 재구축을 반복하면 원본에 부하만 줍니다.** 감시가 충분히 정확하다고 확인된 뒤에 켤 일이며, 그전까지는 사람이 로그를 보고 관리 엔드포인트로 부르는 편이 안전합니다.

순서가 있습니다 — **먼저 감지, 그다음 자동 복구.** 반대로 하면 잘못된 감지가 잘못된 복구를 부릅니다.

### 계산 규칙이 코드로 묶이지 않는 문제

체크섬은 **양쪽이 정확히 같은 규칙으로 계산**해야 합니다. `BigDecimal`의 scale(`89000` vs `89000.00`) 하나만 어긋나도 값이 같은데 영원히 불일치로 나옵니다.

그런데 이 저장소는 공통 모듈을 두지 않습니다. 규칙이 **코드가 아니라 문서로만** 묶입니다. 메시지 record 중복과 달리 여기서는 **필드 이름이 아니라 계산 로직**이 걸리므로 어긋나기 더 쉽습니다.

대응은 **양쪽 테스트에 같은 입력의 같은 기대값을 고정**하는 것입니다.
...
한쪽 규칙이 바뀌면 그쪽 빌드가 깨져 드러납니다. 공통 모듈이 없는 대가를 **빌드로 메우는** 방법입니다.

### 코드

| 파일 | 역할 |
|---|---|
| `product-service/.../Product.version` | 변경 순번. 바뀔 때마다 +1 |
| `product-service/.../ProductChecksum.java` | 요약값 계산. **규칙이 문서로 고정됨** |
| `product-service/.../ProductController.checksum()` | `GET /products/checksum` |
| `order-service/.../ProductChecksum.java` | 같은 규칙의 복제본 쪽 계산 |
| `order-service/.../ProductReplicaVerifier.java` | 주기 대조 + 오탐 처리 |
| `order-service/.../ProductChangedListener` | 갭 감지, 중복·역전 무시 |

### 직접 확인

Kafka만 멈춰 복제 경로를 끊고 상품을 하나 등록합니다. product-service는 살아 있으므로 체크섬 조회는 계속 됩니다.
...
임계(3회 × 15초)를 넘기면 경고가 나옵니다. **원본 4건 / 복제본 3건까지 로그에 남습니다.**

```
WARN  복제본이 원본과 계속 다르다. 재구축이 필요하다
      (연속 3회, 원본 4건/32e939a1..., 복제본 3건/d83687fc...)
```

Kafka를 살리면 outbox에 쌓인 이벤트가 나가고 복제본이 따라잡습니다.

```
INFO  복제본이 다시 원본과 일치한다 (연속 불일치 5회 뒤 회복)
```

**오탐이 걸러지는 것도 실측됩니다.** 기동 직후 복제본이 아직 비어 있는 동안에도 불일치가 잡히지만, 임계에 이르기 전에 회복되어 **경고가 뜨지 않습니다.**

```
INFO  복제본이 다시 원본과 일치한다 (연속 불일치 2회 뒤 회복)
```

같은 "불일치"인데 하나는 조용히 지나가고 하나는 경고가 됩니다. **그 차이를 만드는 것이 임계값입니다.**

### 남은 것

**검증은 "다르다"까지만 말합니다.** 어디가 다른지는 모르므로 대응이 전체 재구축뿐입니다. 범위별 체크섬(Merkle tree 방식)으로 좁혀 가면 부분 복구가 가능하지만, 상품이 3행인 여기서는 과합니다.

그리고 **재구축은 토픽을 진실로 삼습니다.** 토픽 자체가 오염되면(잘못된 이벤트가 발행되면) 재구축으로도 고쳐지지 않습니다. 그때는 원본에서 다시 긁어오는 경로가 따로 필요합니다.
...
| `payment-service` 까지 Outbox 확대 | order·product 에 적용했다(13·14절). payment 는 아직 리스너에서 직접 발행한다 | 결제 응답도 유실되면 안 될 때 |
| CQRS 읽기 모델(별도 저장소) | 로컬 복제본으로 충분했다(14절). 기제는 같고 저장 위치만 다르다 | 가로지르는 화면이 여러 개로 늘고 조회 부하를 따로 감당해야 할 때 |
| blue-green 재구축 | 복제본이 3행이라 재구축이 순간이다(15절) | 재구축이 오래 걸려 그동안 조회를 멈출 수 없을 때 |
| 불일치 지점 특정 | 검증은 "다르다"까지만 말한다(16절). 대응이 전체 재구축뿐이다 | 범위별 체크섬으로 부분 복구가 필요할 때 |
| 불일치 시 자동 재구축 | 감지가 충분히 정확하다고 확인된 뒤에 켤 일이다(16절) | 사람의 개입 없이 스스로 복구해야 할 때 |
| 오염된 토픽 복구 | 재구축은 토픽을 진실로 삼는다. 토픽이 틀리면 못 고친다 | 원본에서 다시 긁어오는 경로가 필요할 때 |
| CDC 기반 릴레이 | 폴링으로 충분했다 | 폴링 지연이 문제가 될 때 |
